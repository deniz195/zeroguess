...
def test_installation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        run_command(
            [venv_python, "-m", "pip", "install", "--force-reinstall", str(wheel_path)]
        )
...
def upload_to_test_pypi():
    ...
    print(f"pip install --index-url https://test.pypi.org/simple/ zeroguess")
...
def check_version_consistency():
    ...
    try:
        ...
        if pyproject_version != package_version:
            print(f"ERROR: Version mismatch! pyproject.toml has version {pyproject_version} but "
...
def main():
    ...
    parser.add_argument(
        "--upload-test", 
        action="store_true", 
        help="Upload the built packages to Test PyPI"
    )
    parser.add_argument(
        "--test-install", 
        action="store_true", 
        help="Test installation from the built wheel"
    )
    parser.add_argument(
        "--deep-clean", 
        action="store_true", 
        help="Perform a deeper clean including cache files"
    )
    parser.add_argument(
        "--skip-version-check",
        action="store_true",
