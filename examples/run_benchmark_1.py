...
def visualize_fit(wavelet, x_data, y_data, y_true, true_params, initial_params, fitted_params, 
    ...
    if success:
        ...
        status_text = "FAILURE"
        status_color = "red"
    ax_main.text(0.05, 0.95, status_text, transform=ax_main.transAxes,
...
def run_lmfit_comparison_benchmark(n_samples=50, noise_level=0.05, tolerance=DEFAULT_TOLERANCE):
    ...
    lmfit_methods = ['leastsq', 'least_squares', 'differential_evolution', 'brute', 'basinhopping', 
          ...
          'tnc', 'trust-ncg', 'trust-exact', 'trust-krylov', 'trust-constr', 
          'dogleg', 'slsqp', 'emcee', 'shgo', 'dual_annealing']
    ...
    for i, true_params in enumerate(param_sets):
        ...
        all_methods = [(m[0]+f" ({lmfit_method})", m[1], lmfit_method, m[2]) for lmfit_method in lmfit_methods for m in base_methods]
...
def generate_lmfit_comparison_report(results_df, output_dir, tolerance=DEFAULT_TOLERANCE):
    ...
    # Plot success rates
    ax1 = plt.subplot(121)
    method_summary[['fit_success', 'param_success']].plot(
        kind='bar', ax=ax1, color=['#4e79a7', '#f28e2b']
    )
