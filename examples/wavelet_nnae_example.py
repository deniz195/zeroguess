#!/usr/bin/env python
"""
Example demonstrating ZeroGuess's Neural Network Autoencoder Estimator (NNAE)
for parameter estimation with a wavelet function.

This example shows how to use the NNAE estimator for parameter estimation and
compares its performance with the standard neural network estimator and
traditional curve fitting.

The true parameters are randomly generated and the same noisy dataset is used
for all methods to ensure direct comparison.
