...
class NNAEEstimator(BaseEstimator):
    ...
    def train(self, n_samples=10000, batch_size=64, epochs=50, n_epochs=None, validation_split=0.2, 
        ...
        print(f"Encoder: {encoder_epochs} epochs, Decoder: {decoder_epochs} epochs, End-to-End: {end_to_end_epochs} epochs")
        ...
        encoder_train_dataset = TensorDataset(
            ...
            torch.tensor(params_train, dtype=torch.float32)
        )
        encoder_val_dataset = TensorDataset(
            torch.tensor(y_val, dtype=torch.float32),
            torch.tensor(params_val, dtype=torch.float32)
        )
        ...
        # Create data loaders
        encoder_train_loader = DataLoader(
            encoder_train_dataset, batch_size=batch_size, shuffle=True
        )
        encoder_val_loader = DataLoader(
            encoder_val_dataset, batch_size=batch_size, shuffle=False
        )
        
        decoder_train_loader = DataLoader(
            decoder_train_dataset, batch_size=batch_size, shuffle=True
        )
        decoder_val_loader = DataLoader(
            decoder_val_dataset, batch_size=batch_size, shuffle=False
        )
        
        end_to_end_train_loader = DataLoader(
            end_to_end_train_dataset, batch_size=batch_size, shuffle=True
        )
        end_to_end_val_loader = DataLoader(
            end_to_end_val_dataset, batch_size=batch_size, shuffle=False
        )
    ...
    def predict(self, x=None, y=None, return_network_output=False):
        ...
        Args:
            ...
            return_network_output: If True, return both the parameters and network outputs (normalized params and moments).
        ...
        if return_network_output:
            network_outputs = {
                'normalized_params': params.cpu().numpy(),
...
class _ResidualDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, (layer, bn, dropout, res_proj) in enumerate(zip(
            self.layers, self.batch_norms, self.dropouts, self.residual_projections
        )):
